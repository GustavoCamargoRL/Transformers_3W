...
def load_seu(path_to_folder, window_length=1024, preprocessing=None,
    ...
    for file in dataset_files:
        # Some files have tab as separator, some have comma, so handle both
        loaded = pd.read_parquet(file, skiprows=17, sep='[,\\t]', header=None,
...
def load_3w_v2(path_to_folder, window_length=64, preprocessing=None,
    ...
    """
    # Getting data files
    dataset_files = glob(f'{path_to_folder}/[0-9]/*.parquet', recursive=True)

    # Cases considered
    if single_class is not None:
        cases = [str(single_class)]
    else:
        cases = [str(i) for i in range(10)]  # Agora vai de 0 a 9
    ...
    for file in dataset_files:
        # Ignorar arquivos da pasta folds
        if 'folds' in file:
            continue

        # Ignorar arquivos das classes excluídas
        if any(ex in file for ex in exclude):
            continue

        # Identificar a classe pelo diretório pai
        class_label = os.path.basename(os.path.dirname(file))
        if class_label not in cases:
            continue

        try:
            loaded = pd.read_parquet(file)
            # Remove linhas onde a penúltima coluna é N/A
            col_class = loaded.columns[-2]
            loaded = loaded[loaded[col_class].notna()]
        except Exception as e:
            print(f"Erro ao ler {file}: {e}")
            continue
    ...
    for case in cases_dict:
        try:
            data = np.concatenate(cases_dict[case], axis=0)
            if len(data) != 0:
                if case == 0.0 or case == '0.0':
                    cases_list.insert(0, data)
                else:
    ...
    if len(cases_list) == 0:
        raise ValueError("Nenhum dado foi carregado. Verifique o caminho do dataset e o formato dos arquivos.")

    return organize_train_test(cases_list, window_length,
                               preprocessing, test_size, scaler, dataframes=None)
